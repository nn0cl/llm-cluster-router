...
これはルートの `README.ja.md` の逐語訳ではありません。日本語話者がこの
テンプレートを導入・運用するときに、判断を迷いやすい点を短く確認するための
入口です。製品説明はルートの `README.ja.md` を参照してください。
...
## llm-cluster-router との関係

`llm-cluster-router` は、このテンプレートの標準ストラテジーを適用した
具体的なプロダクト例です。上流テンプレートは
[nn0cl/llm-project-template](https://github.com/nn0cl/llm-project-template)
です。

- AT-TDD と Clean Architecture の境界ルールを、このテンプレートに従って運用。
- provider・設定・出力パスなどの外部リソースを port として扱う。
- AI request routing と reasoning contract を `docs/architecture/` 配下に明示。

テンプレートそのものを理解したい場合は、この文書を読み、実際の適用例として
ルートの `README.ja.md` と本リポジトリのコードを見る順番がおすすめです。
